...
def get_html_files(site_dir: Path) -> list:
    """Get all non-redirect index.html files from _site/."""
    files = []
    for f in sorted(site_dir.glob("**/index.html")):
        content = f.read_text()
        # Skip redirect pages generated by jekyll-redirect-from
...
class TestHTMLBasicStructure:
    ...
    def test_all_pages_have_description_meta(self, html_files, site_dir):
        """Test that main content pages have meta description (og:description)."""
        # Archive/utility pages (categories, tags, etc.) don't get og:description
        # because seo.html only outputs it when page.excerpt is set.
        # Only check pages we control that have excerpt/description front matter.
        checked_pages = {
            site_dir / "index.html",
            site_dir / "research" / "index.html",
            site_dir / "experience" / "index.html",
            site_dir / "projects" / "index.html",
            site_dir / "skills" / "index.html",
        }
        for html_file in html_files:
            if html_file not in checked_pages:
                continue
            soup = BeautifulSoup(html_file.read_text(), "html.parser")
            desc = (
        ...
        for html_file in html_files:
            if "index.html" == html_file.name and html_file.parent == site_dir:
                ...
                for expected_url in expected_nav_urls:
                    # Accept both relative (/path/) and absolute (https://host/path/) hrefs
                    found = any(
                        href == expected_url or href.endswith(expected_url)
